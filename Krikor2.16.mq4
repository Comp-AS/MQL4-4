...
//Wat gebeurt er als ik tussentijds variabelen verander? Reset hij zich dan? Uittesten
 
//Nieuwschecker inbouwen die rekening houdt met nieuwsmomenten. Als op moment van nieuws met categorie High de trend de verkeerde
// kant op gaat, Grid verdubbelen. Als trend de goede kant op gaat, niets aan doen.
// Beschrijving: https://www.mql5.com/en/articles/1502
// andere opties:
// http://forexmt4ea.com/news-calendar-indicator-mt4/
// http://www.forexfactory.com/showthread.php?t=326551
// http://www.forexfactory.com/showthread.php?t=19293

//Stukje toevoegen dat ook handmatig gestart kan worden met een trade

//Controleren of er meer profit/minder DD te halen is met verschillende standen van de indicatoren, vooral ZigZag. Daarvoor dmv een case
// statement de verschillende tijden omzetten naar 1 t/m 6 (ofzo) om te kunnen doorlopen met de optimization

//2015.01.28 15:46:12.751  '1028745': order sell market 0.01 USDCHF sl: 0.00000 tp: 0.00000
//2015.01.28 15:44:20.284 '1028745': order #80618797 sell 0.01 USDCHF closing at 0.00000 failed [Invalid parameters]
//2015.01.28 15:46:12.751  '1028745': order sell market 0.01 USDCHF sl: 0.00000 tp: 0.00000
//2015.01.28 15:44:20.284 '1028745': order #80618797 sell 0.01 USDCHF closing at 0.00000 failed [Invalid parameters]

//2015.01.28 16:14:01.871  '1028750': order sell 0.04 GBPCHF opening at market sl: 0.00000 tp: 0.00000 failed [Not enough money]
//Toch ook eens kijken naar dat - not enough money - ding.
...
extern double RiskPercent        =0.5;            //Risk percentage
extern double FixedLots          =0.1;          //Fixed Lotsize
extern bool UseMaxDDPerc         =false;        //Use MaxDD percentage
extern double MaxDrawDownPercent =20;           //Max drawdown percentage
extern bool UseMaxDDFixed        =true;         //Use MaxDD fixed amount
extern double MaxDrawDownFixed   =200;          //Max drawdown fixed amount
extern int MaxTrades             =6;            //Max number of trades 
extern int CashIn                =3;            //Cashen als trade goed gaat
extern int period                =1;            //Periode voor trade entry

extern string Grid_settings      ="Grid settings section";
extern double GridPosMultiplier  =1;          //Grid positive multiplier
...
extern double GridDivider        =19;            //Bij Grid optellen om te delen
extern double GridMultiplier     =1;            //Vermenigvuldigingsfactor

extern bool UseHedge             =false;        //Use hedge function?

extern string MA                 ="Moving Average";
extern int MAPeriod              =1440;         //MA Periode
...
extern string Nieuws             = "Nieuws filter";
extern bool NieuwsFilter         =true;
extern int MinsBeforeNews        = 60;         //Minutes before news
extern int MinsAfterNews         = 60;         //Minutes after news
bool NewsTime;
...
//MA variabelen
double H1MA20, H1MA50;
string MAcomp;

//Average daily range variabelen
double R1=0,R5=0,R10=0,R20=0,RAmax1=0,RAmax2=0,RAmax=0;

//ADX variabelen
double ADXmain, ADXplusDI, ADXminDI;
...
double FirstOrderPrice=0;              //Prijs van de first order
double Geld;                           //Accountalance placeholder om dominoeffect te voorkomen
string Commentaar="";                  //Commentaar in geval van nieuws
int MaximumTrades;                     //Maximum trades placeholder
...
void OnDeinit(const int reason)
  {
   ...
   ObjectDelete("FirstOrderType");
   ObjectDelete("FirstOrderPrice");
   ObjectDelete("Commentaar");
  }
...
void OnTick()
  {
   ...
   //Hiermee zou hij door moeten kunnen gaan in geval van een restart
   if(NumberOfTrades()!=0){
      Start=false;
      if(FirstOrderPrice==0 || FirstOrderType==""){
   ...
   //Eerst kijken of er nieuws is, hij kijkt eens per minuut
   if(NieuwsFilter){
      static int PrevMinute = -1;
      if(Minute() != PrevMinute){
         PrevMinute = Minute();
       
         int minutesSincePrevEvent = iCustom(Symbol(), 0, "FFCal", true, false, false, true, true, 1, 0);
         int minutesUntilNextEvent = iCustom(Symbol(), 0, "FFCal", true, false, false, true, true, 1, 1);
            NewsTime = false;
      ...
      if(!Start && NewsTime){
         if(NumberOfTrades() <= 3 && CurrentProfit()>0.15*TargetProfit()){
            TradeExit(); Start=true; 
            Comment("Trading gestopt door nieuws"); 
            Commentaar="Trading gestopt door nieuws"; 
            return;
         }
         else if(CurrentProfit()<0 && NumberOfTrades() > 3){
               MaximumTrades=NumberOfTrades(); 
               GridValue=NormalizeDouble(RAmax/(MaxTrades),Digits); 
               GridValue+=NormalizeDouble(NumberOfTrades()*GridMultiplier*pips,Digits);
               Comment("Grid gemaximaliseerd en MaxTrades: " + IntegerToString(NumberOfTrades())); 
               Commentaar="Grid gemaximaliseerd en MaxTrades: " + IntegerToString(NumberOfTrades());
         }
         else{
            GridValue=NormalizeDouble(RAmax/(MaxTrades),Digits); 
            GridValue+=NormalizeDouble(NumberOfTrades()*GridMultiplier*pips,Digits);
            Comment("Grid gemaximaliseerd"); Commentaar="Grid gemaximaliseerd";
         }      
      }
      else if(Start && NewsTime){
         Comment("Trading gestopt door nieuws"); 
         Commentaar="Trading gestopt door nieuws"; 
         return;
      }
      else{Comment("Geen mededelingen"); Commentaar="Geen mededelingen";}
   }
   ...
   //+------------------------------------------------------------------+
   //MA indicatoren   
   H1MA20 = iMA(Symbol(),MAPeriod,20,0,MODE_SMA,PRICE_CLOSE,1);
   H1MA50 = iMA(Symbol(),MAPeriod,50,0,MODE_SMA,PRICE_CLOSE,1);
   ...
   //+------------------------------------------------------------------+
   RSImain=iRSI(Symbol(),period,RSIperiod,PRICE_CLOSE,0);
          
   //+------------------------------------------------------------------+
   // Average Directional Movement Index
   //+------------------------------------------------------------------+
      
   ADXmain=iADX(Symbol(),period,ADXperiod,PRICE_CLOSE,MODE_MAIN,1);
   ADXplusDI=iADX(Symbol(),period,ADXperiod,PRICE_CLOSE,MODE_PLUSDI,1); 
   ADXminDI=iADX(Symbol(),period,ADXperiod,PRICE_CLOSE,MODE_MINUSDI,1);
   ...
   if(Start){
      ...
      if(ADXplusDI>25 && ADXmain>25 && RSImain<85){
         if((time0+75)<timeNu){
      ...
      if(ADXminDI>25  && ADXmain>25 && RSImain>15){
         if((time0+75)<timeNu)  {
            if(Volume[0]>(1*gemVolume)) {
               if(H1MA20<H1MA50){
   ...
   if(!Start){
      if(FirstOrderType=="OP_BUY"){
         ...
         if(Ask<=FirstOrderPrice-(NumberOfTrades()*GridValue)){
            if(NumberOfTrades()<MaximumTrades-1){
               TradeEntry(0,GLots(GridNegMultiplier),IntegerToString(NumberOfTrades()));
            }
            if(UseHedge){
            if(NumberOfTrades()==MaximumTrades-1 && Ask<=((FirstOrderPrice-(NumberOfTrades())*GridValue))){
               Hedge=true;
      ...
      if(FirstOrderType=="OP_SELL"){
         ...
         if(Bid>=FirstOrderPrice+(NumberOfTrades()*GridValue)){
            ...
            if(UseHedge){
            if(NumberOfTrades()==MaximumTrades-1 && Bid>=((FirstOrderPrice+(NumberOfTrades())*GridValue))){
               Hedge=true;
   ...
   if(Hedge){
      if(FirstOrderType=="OP_BUY"){
         if(Ask>=FirstOrderPrice-((MaximumTrades-2)*GridValue)){
            TradeExit(); Hedge=false;
         }
      }
      if(FirstOrderType=="OP_SELL"){
         if(Bid<=FirstOrderPrice+((MaximumTrades-2)*GridValue)){
            TradeExit(); Hedge=false;
         }
      }
   }
   ...
   if(UseMaxDDPerc){
      if(CurrentProfit()<0){DrawDown=((CurrentProfit()*-1)/Geld)*100;
         if(MaxDD<DrawDown){MaxDD=DrawDown;}
         if(DrawDown>MaxDrawDownPercent){TradeExit();}
      }
   }
   if(UseMaxDDFixed){
      if(CurrentProfit()<0){DrawDown=CurrentProfit()*-1; 
         if(MaxDD<DrawDown){MaxDD=DrawDown;}
         if(DrawDown>MaxDrawDownFixed){TradeExit();}
      }
   }
...
//+-----------------------------------------------------------------+
//| Get first order details in geval van restart
//+-----------------------------------------------------------------+

void GetFirstOrderDetails(){
 
   for(i=OrdersTotal(); i>=0; i--){
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderMagicNumber()==Magic && OrderSymbol()==Symbol()){
         if(OrderComment()=="0"){
            if(OrderType()==OP_BUY){FirstOrderType="OP_BUY";}
            if(OrderType()==OP_SELL){FirstOrderType="OP_SELL";}
...
//Checking account margin
if(AccountFreeMarginCheck(Symbol(),cmd,volume)<=0){
   Print("Niet genoeg marge! Symbol: " + Symbol() + "Commando: " + sp + IntegerToString(cmd) + sp + "Volume: " + DoubleToStr(volume,2));
   return;
}
   ...
   if(cmd==0){
      for(i=1;i>0;i--){
         ...
         if(ticket>0){i=0;Print("OrderSend OP_BUY - Bid: " + DoubleToStr(Bid,Digits) + sp + "GLots: " + DoubleToStr(volume,2) + sp + "Comment: " + comment);}
         else{     
            Print("Error OrderSend OP_BUY: " + DoubleToStr(GetLastError(),0) + sp + "Ask: " + DoubleToStr(Ask,Digits) + sp + "GLots: " + DoubleToStr(volume,2) + sp + "Comment: " + comment);
   ...
   if(cmd==1){
      for(i=1;i>0;i--){
         ...
         if(ticket>0){i=0; Print("OrderSend OP_SELL - Bid: " + DoubleToStr(Bid,Digits) + sp + "GLots: " + DoubleToStr(volume,2) + sp + "Comment: " + comment);}
         else{     
            Print("Error OrderSend OP_SELL: " + DoubleToStr(GetLastError(),0) + sp + "Bid: " + DoubleToStr(Bid,Digits) + sp + "GLots: " + DoubleToStr(volume,2) + sp + "Comment: " + comment);
            if(retry==maxretry){Print("Maxretry reached, stopped trying"); break;}
            err=GetLastError();
            switch(err){
               case ERR_SERVER_BUSY:
...
//+------------------------------------------------------------------+
//| ZigAndZag Slalom With A Scalpel                                                                 |
//+------------------------------------------------------------------+
...
void Comments(){
   ...
   ObjectCreate("RSImain",OBJ_LABEL,0,0,0,0,0);
   ...
   ObjectSetText("Version", "Version: 2.16", Font, "Arial",MenuColor);
   ...
   ObjectSetText("RSImain", "RSImain: " + DoubleToStr(RSImain,2), Font, "Arial",MenuColor);
   ObjectSet("RSImain",OBJPROP_XDISTANCE,x_afstand);     
   ObjectSet("RSImain",OBJPROP_YDISTANCE,y_afstand+65);
   ObjectSet("RSImain",OBJPROP_CORNER,1);
   ...
   string percentage="";
   if(UseMaxDDPerc){percentage="%";}
   else{percentage="";}
   ObjectSetText("MaxDD", "MaxDD: " +  DoubleToStr(MaxDD,2) + percentage, Font, "Arial",MenuColor);
   ...
   ObjectCreate("FirstOrderType",OBJ_LABEL,0,0,0,0,0);
   ObjectSetText("FirstOrderType", "FirstOrderType: " + FirstOrderType, Font, "Arial",MenuColor);
   ObjectSet("FirstOrderType",OBJPROP_XDISTANCE,x_afstand);     
   ObjectSet("FirstOrderType",OBJPROP_YDISTANCE,y_afstand+200);
   ObjectSet("FirstOrderType",OBJPROP_CORNER,1);
   
   ObjectCreate("FirstOrderPrice",OBJ_LABEL,0,0,0,0,0);
   ObjectSetText("FirstOrderPrice", "FirstOrderPrice: " + DoubleToStr(FirstOrderPrice,Digits), Font, "Arial",MenuColor);
   ObjectSet("FirstOrderPrice",OBJPROP_XDISTANCE,x_afstand);     
   ObjectSet("FirstOrderPrice",OBJPROP_YDISTANCE,y_afstand+215);
   ObjectSet("FirstOrderPrice",OBJPROP_CORNER,1);
   
   if(Commentaar != ""){
      ObjectCreate("Commentaar",OBJ_LABEL,0,0,0,0,0);
      ObjectSetText("Commentaar", "Commentaar: " + Commentaar, Font, "Arial",MenuColor);
      ObjectSet("Commentaar",OBJPROP_XDISTANCE,x_afstand);     
      ObjectSet("Commentaar",OBJPROP_YDISTANCE,y_afstand+230);
      ObjectSet("Commentaar",OBJPROP_CORNER,1);
   }
}