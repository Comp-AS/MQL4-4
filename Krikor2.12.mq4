...
void OnTick()
  {
            ...
            if(Volume[0]>(1*gemVolume)){
               if(H1MA20>H1MA50){
                   if(Porog3[0]!=0 || BuyOrdersBuffer[0]!=0){
                     ...
                     Start=false; GLots=Lots; FirstOrderPrice=Ask; FirstOrderType="OP_BUY";
                  }
               }
            }
         }
            ...
            if(Volume[0]>(1*gemVolume)) {
               if(H1MA20<H1MA50){
                  if(Porog2[0]!=0 || SellOrdersBuffer[0]!=0){
                     ...
                     Start=false; GLots=Lots; FirstOrderPrice=Bid; FirstOrderType="OP_SELL";
                  }
               }
            }
         }
      ...
      if(FirstOrderType=="OP_BUY"){
         if(Ask>=FirstOrderPrice+(NumberOfTrades()*GridValue)){
            ...
            //If not, continue to CashIn
               ...
               TradeEntry(0,GLots,IntegerToString(NumberOfTrades()));
            }
         }
         if(Ask<=FirstOrderPrice-(NumberOfTrades()*GridValue)){
      ...
      if(FirstOrderType=="OP_SELL"){
         if(Bid<=FirstOrderPrice-(NumberOfTrades()*GridValue)){
            ...
            //If not, continue to CashIn
               ...
               GLots=NormalizeDouble(GLots*GridPosMultiplier,2);
               if(GLots<MarketInfo(Symbol(),MODE_MINLOT)){GLots=MarketInfo(Symbol(),MODE_MINLOT);}
               TradeEntry(1,GLots,IntegerToString(NumberOfTrades()));
            }
            }
         if(Bid>=FirstOrderPrice+(NumberOfTrades()*GridValue)){
            if(NumberOfTrades()<MaxTrades-1){
   ...
   ObjectSetText("Version", "Version: 2.12", Font, "Arial",MenuColor);
