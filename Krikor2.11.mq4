...
//In het gedeelte voor het zenden van orders, moet het niet zijn for(i=1;i>=0;i--){ ? Even checken.
...
extern int period                =5;            //Periode voor trade entry
...
double GLots;                          //Lots variabele gebruikt voor koop/verkoop zelf
...
void OnTick()
  {
   ...
   //+------------------------------------------------------------------+
   //MA indicatoren   
   double H1MA20 = iMA(Symbol(),PERIOD_H1,20,0,MODE_SMA,PRICE_CLOSE,1);
   double H1MA50 = iMA(Symbol(),PERIOD_H1,50,0,MODE_SMA,PRICE_CLOSE,1);
   ...
   Porog1[1] = Const5;
   int brshift = bars2-1;
   //ArrayResize(Porog1,brshift);
   //ArrayResize(Porog2,brshift);
   //ArrayResize(Porog3,brshift);
   ...
   shift=ZigAndZagMaxBars-1;
   //ArrayResize(SlalomZagBuffer,shift);
   //ArrayResize(SlalomZigBuffer,shift);
   //ArrayResize(KeelOverZagBuffer,shift);
   //ArrayResize(KeelOverZigAndZagSECTION,shift);
   //ArrayResize(LimitOrdersBuffer,shift);
   //ArrayResize(BuyOrdersBuffer,shift);
   //ArrayResize(SellOrdersBuffer,shift);
   
   for(shift; shift>240-KeelOver; shift--){   
            ...
            if(Volume[0]>(1*gemVolume)){
               if(H1MA20>H1MA50){
                  if(Porog3[0]!=0 || BuyOrdersBuffer[0]!=0){
                     TradeEntry(0,Lots,IntegerToString(0) + "^" + DoubleToStr(Ask,Digits) + "*" + DoubleToStr(GridValue,Digits));
                     Start=false;
                     GLots=Lots;
                  }
               }
            ...
            if(Volume[0]>(1*gemVolume)) {
               if(H1MA20<H1MA50){
                  if(Porog2[0]!=0 || SellOrdersBuffer[0]!=0){
                     TradeEntry(1,Lots,IntegerToString(1) + "^" + DoubleToStr(Bid,Digits) + "*" + DoubleToStr(GridValue,Digits));
                     Start=false;
                     GLots=Lots;
                  }
               }
   ...
   if(!Start){
      if(FirstOrderType()==0){
         if(Ask>=FirstOrderPrice()+(NumberOfTrades()*GridValue)){
            GLots=NormalizeDouble(GLots*GridPosMultiplier,2);
            if(GLots<MarketInfo(Symbol(),MODE_MINLOT)){GLots=MarketInfo(Symbol(),MODE_MINLOT);}
            TradeEntry(0,GLots,IntegerToString(0) + "^" + DoubleToStr(FirstOrderPrice(),Digits) + "*" + DoubleToStr(GridValue,Digits));
         }
         if(Ask<=FirstOrderPrice()-(NumberOfTrades()*GridValue)){
            if(NumberOfTrades()<MaxTrades-1){
               GLots=NormalizeDouble(GLots*GridNegMultiplier,2); 
               if(GLots>MarketInfo(Symbol(),MODE_MAXLOT)){GLots=MarketInfo(Symbol(),MODE_MAXLOT);}
               TradeEntry(0,GLots,IntegerToString(0) + "^" + DoubleToStr(FirstOrderPrice(),Digits) + "*" + DoubleToStr(GridValue,Digits));
            }
            if(UseHedge){
            if(NumberOfTrades()==MaxTrades-1 && Ask<=((FirstOrderPrice()-(NumberOfTrades())*GridValue))){
               GLots=NormalizeDouble(GLots*GridNegMultiplier*4,2); Hedge=true;
               if(GLots>MarketInfo(Symbol(),MODE_MAXLOT)){GLots=MarketInfo(Symbol(),MODE_MAXLOT);}
               TradeEntry(0,GLots,IntegerToString(1) + "^" + DoubleToStr(FirstOrderPrice(),Digits) + "*" + DoubleToStr(GridValue,Digits));
            }
            }
         }
      }
      if(FirstOrderType()==1){
         if(Bid<=FirstOrderPrice()-(NumberOfTrades()*GridValue)){
            GLots=NormalizeDouble(GLots*GridPosMultiplier,2);
            if(GLots<MarketInfo(Symbol(),MODE_MINLOT)){GLots=MarketInfo(Symbol(),MODE_MINLOT);}
            TradeEntry(1,GLots,IntegerToString(1) + "^" + DoubleToStr(FirstOrderPrice(),Digits) + "*" + DoubleToStr(GridValue,Digits));
            }
         if(Bid>=FirstOrderPrice()+(NumberOfTrades()*GridValue)){
            if(NumberOfTrades()<MaxTrades-1){
               GLots=NormalizeDouble(GLots*GridNegMultiplier,2); 
            ...
            if(NumberOfTrades()==MaxTrades-1 && Bid>=((FirstOrderPrice()+(NumberOfTrades())*GridValue))){
               GLots=NormalizeDouble(GLots*GridNegMultiplier*4,2); Hedge=true;
               if(GLots>MarketInfo(Symbol(),MODE_MAXLOT)){GLots=MarketInfo(Symbol(),MODE_MAXLOT);}
               TradeEntry(1,GLots,IntegerToString(0) + "^" + DoubleToStr(FirstOrderPrice(),Digits) + "*" + DoubleToStr(GridValue,Digits));   
            }
            }
         }   
      }
   ...
   //+------------------------------------------------------------------+
   // Condities voor trade exit
   //+------------------------------------------------------------------+
    
   if(NumberOfTrades()>=3 && CurrentProfit()>MinProfitValue*TargetProfit() && CurrentProfit()<TargetProfit()*MaxProfitValue){
   TradeExit();}
   if(NumberOfTrades()==MaxTrades && CurrentProfit()>TargetProfit()*MaxProfitValue){TradeExit();}
   if(Hedge){
      if(FirstOrderType()==0){
   ...
   ObjectSetText("Version", "Version: 2.11", Font, "Arial",MenuColor);
   ...
   Comment("\n" + "\n" +
            "\n" + "Magisch nummer: " + IntegerToString(Magic) +
            "\n" + "FirstOrderType: " + IntegerToString(FirstOrderType(),1) +
            "\n" + "FirstOrderPrice: " + DoubleToStr(FirstOrderPrice(),Digits) +
            "\n" + "ZigZagLarsen: " + ZigZagLarsen +
            "\n" + "ZigAndZag: " + ZigAndZag);
}         
...
if(MarketInfo(OrderSymbol(),MODE_MARGINCALCMODE)==0){LotSize=LotSize/100000;}
   ...
   for(i=OrdersTotal(); i>=0;i--){
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol()==Symbol() && OrderMagicNumber()==Magic){
         lengte=StringLen(OrderComment())-StringFind(OrderComment(),"*",0)-1;
         if(OrderType()==OP_BUY){target=OrderOpenPrice()+StringToDouble(StringSubstr(OrderComment(),StringFind(OrderComment(),"*",0)+1,lengte));}
         if(OrderType()==OP_SELL){target=OrderOpenPrice()-StringToDouble(StringSubstr(OrderComment(),StringFind(OrderComment(),"*",0)+1,lengte));}
         
         test=StringToDouble(StringSubstr(OrderComment(),Digits+5,Digits+2));
         
         double RiskInPoints=MathAbs((OrderOpenPrice()-target)/TickSize);
...
double ReturnValue=0;
   for(i=OrdersTotal(); i>=0;i--){
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol()==Symbol() && OrderMagicNumber()==Magic){
         lengte=StringFind(OrderComment(),"*",0)-StringFind(OrderComment(),"^",0)-1;
         ReturnValue=StringToDouble(StringSubstr(OrderComment(),StringFind(OrderComment(),"^",0)+1,lengte));
      }
   }
return(ReturnValue);
}
...
int ReturnValue=0;
   for(i=OrdersTotal(); i>=0;i--){
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderSymbol()==Symbol() && OrderMagicNumber()==Magic){
         ReturnValue=StringToInteger(StringSubstr(OrderComment(),0,1));
      }
   }
return(ReturnValue);
}
...
////+-----------------------------------------------------------------+
//int new_candle() {
//   if (mdn != iTime(Symbol(),ZigZagLarsenPeriode,0) || mdl != iBars(Symbol(),ZigZagLarsenPeriode)) {
//      mdn = iTime(Symbol(),ZigZagLarsenPeriode,0);
//      mdl = iBars(Symbol(),ZigZagLarsenPeriode);
//      return (1);
//      }
//return (0);
//}
